...
export const GroupManagement: React.FC<{ groupId: string | undefined }> = ({
  groupId,
}) => {
  const { data, isLoading, isError, error } = useFetchGroup(groupId as string);
  const { data: isAdmin, isLoading: roleLoading } = useCheckRole(
    groupId as string
  );
  ...
  return (
    <>
      ...
      {roleLoading && <Loading size={50} />}
      {isAdmin && <ButtonArea groupId={groupId} />}
    </>
  );
};
